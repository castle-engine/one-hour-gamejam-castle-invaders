...
pipeline {
  ...
  stages {
    stage('Build Desktop') {
      steps {
        sh 'rm -f castle_invaders-*.tar.gz castle_invaders-*.zip castle_invaders*.apk'
  ...
  post {
    success {
      archiveArtifacts artifacts: 'castle_invaders*.tar.gz,castle_invaders*.zip,castle_invaders*.apk'
    }
    regression {
